# Tideproof visual and release system
...
This document is the visual source of truth for Tideproof's website, browser
...
Tideproof should feel upscale, cohesive, metallic, strong, powerful, premium,
and trustworthy. Its relationship to the parent brand is quiet and exact:

> A TrustAgentic.ai project

Ocean, tide, wave, tide-pool, shoreline, and beach imagery may express
vastness, majesty, reliability, controlled force, and resilience. The visual
story is not a disaster simulation and must not imply that Tideproof is
operational emergency software.
...
high-trust tone where appropriate, but Tideproof must not reuse proprietary
...
- generic storm-disaster imagery, frightened people, or emergency voyeurism;
- ornamental waves that reduce the product to a beach or surf brand;
...
| Token | Value | Role |
| --- | --- | --- |
| Abyss | `#07100c` | Page field and deepest coastal shadow |
...
| Sea glass | `#9ff0bd` | Verified/pass state only |
| Signal brass | `#ffd28a` | Pending, caution, or constrained proposal |
| Sounding blue | `#9ad7ff` | Recorded evidence and informational links |
| Coral | `#ffb0b0` | Denial, failure, or invalid state |
| Focus light | `#fff09c` | Keyboard focus only |

TrustAgentic orange remains inside the approved TrustAgentic asset. Do not
sample it into a Tideproof gradient, recolor the asset, or use it as a broad
Tideproof theme color.

Metallic character should come from controlled light, fine borders, tonal
depth, and restrained texture on Tideproof-owned surfaces. Never apply fake
...
- Preserve exact casing for provider names, `TrustAgentic.ai`, receipt states,
  and proof labels.
- A future Tideproof wordmark must be a reviewed master asset. Do not ask an
...
- Prefer stable horizons, large negative space, strong geometry, and one
  dominant focal point.
- Use a single image family across website, README, video, and submission,
  derived from the same approved master rather than unrelated stock.
- Keep the persistent proof-state ribbon visually stronger than decorative
  media.
- Do not place important text over detailed surf, specular highlights, or
  moving water.
- Keep the small parent-brand attribution separate from Tideproof's own
  wordmark. It must be readable, but never compete with the product name.

### Motion

- Motion should feel tidal: slow, directional, and inevitable, not floaty or
  frenetic.
...
A reference belongs on the Tideproof moodboard only when it satisfies every
applicable rule below:

1. It conveys scale, controlled force, resilience, precision, or quiet
   reliability.
2. Its palette can live with Abyss, Graphite, Mineral, and the semantic
...
4. Water and shoreline details are physically credible and not sensational.
...
| --- | --- | --- | --- |
| `V01 web/brand/tideproof-wordmark.svg` | Transparent SVG master, horizontal `4:1` artboard; outlined or embedded paths only after a lettering review; export `2048 × 512` transparent PNG | Header, README hero, video cards, submission | `SPEC_ONLY`; Tideproof-owned design and public asset license required |
| `V02 web/brand/tideproof-mark.svg` | Square SVG master with `1024 × 1024` view box; verified exports at `512`, `192`, `180`, `32`, and `16` px | App icon, favicon, social avatar | `SPEC_ONLY`; deliberately omitted from current release surfaces. Any future mark must derive from V01's system, not from the retired interim wave glyph. |
| `V03 web/brand/trustagentic-wordmark-transparent.png` | Use an approved source asset byte-for-byte; current custodian candidate is `900 × 226`, SHA-256 `1d1aec3649f161d7e706c941baa763f73ff480b7ef09072a290373ebf2544b44` | Small “A TrustAgentic.ai project” attribution and video end card only | `RIGHTS_PENDING`; do not copy into Tideproof until Tideproof-specific use and public-repository licensing treatment are recorded |
| `V04 source-private/tideproof-ocean-master` | Owned/licensed still at least `4096 × 2304`; retain original color space and metadata; public derivatives at `2400 × 1350` desktop and `1200 × 1500` mobile in AVIF plus WebP fallback | Website atmosphere, README crop, video establishing frame | `RIGHTS_PENDING`; the private source is never committed, and all public-channel rights must be perpetual or otherwise compatible with the release |
| `V05 web/media/tideproof-hero-*` | V04 derivatives: desktop `16:9` and mobile `4:5`; no text baked in; target each responsive file below `350 KiB` without visible banding | Public website | `SPEC_ONLY`; derivative inherits V04 rights and hash lineage |
| `V06 docs/media/tideproof-readme-hero.png` | `1600 × 900`, sRGB, static, with Tideproof wordmark and short thesis; no metric unless frozen in `CLAIMS.md` | GitHub README and press packet | `SPEC_ONLY`; V01 and V04 must be release-cleared |
| `V07 docs/media/tideproof-og.png` and `tideproof-github-social.png` | Open Graph `1200 × 630`; GitHub social preview `1280 × 640`; safe text inset at least `64` px | Link previews and repository social image | `SPEC_ONLY`; V01 and V04 must be release-cleared |
...
| `V09 docs/media/architecture.svg` and `docs/media/architecture.png` | Vector source matching the browser trust-boundary diagram; embedded accessible title and description plus `ARCHITECTURE.md` text counterpart; exact `2200 × 720` PNG export for non-SVG surfaces | Website, README, video | `CLEARED_CURRENT`; SVG SHA-256 `5e897dbfd926486203362cf517c967e44d799edbf7f56d1d01b16307ec02724c`, PNG SHA-256 `6228172f7a5a462940a05543ee455e0de21aa53c805e9466076b9e33fed1f168`; rights and export provenance in `docs/media/RIGHTS.md` |
...
Neither file is presently stored in Tideproof.
...
Every visual published by Tideproof must have a row in V11 with:
...
Tideproof code. Provider logos should be avoided unless official downloads
...
task. Do not change the production homepage until Tideproof is public, the
exact destination and repository are independently verified, all release
gates are green, and rollback ownership is clear.

## Open owner action

**Brian action needed:** provide one `tideproof-source-art` package containing
(a) the official layered or vector TrustAgentic wordmark and symbol, (b) one
owned or fully licensed original ocean/tide/shoreline still at least
`4096 × 2304`, and (c) a `RIGHTS.txt` granting Tideproof use of those exact
files on the website, public repository, README, demo video, social/press
previews, and hackathon submission while keeping TrustAgentic marks excluded
from Tideproof's MIT license.

Until that package and its hashes are recorded, use text-only parent-brand
attribution and do not generate final marketing art.
