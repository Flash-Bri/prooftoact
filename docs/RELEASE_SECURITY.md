...
| CockroachDB memory and recovery | Before mutation, one read-only SERIALIZABLE v26.2 transaction binds descriptor-backed object and schema/global default privileges in the current dedicated database while recording cache-versioned principal, membership, and default-role introspection. Matching post-convergence and database-set barriers reject drift; the release boundary still requires no concurrent administrator changes. It rejects stale ProofToAct principals and untrusted paths into the managed capability boundary. Exact provider-canonical routine signatures, role grant allowlists, NOLOGIN capability roles, an un-switched bootstrap session, and exact `session_user` guards on every `SECURITY DEFINER` body are attested again after convergence. Runtime URLs cannot override the reviewed query/statement/idle timeouts. Managed MCP uses one fixed query bound to the exact tenant, run, incident, evidence, resource, operation, request digest, outcome, and successor principal. Immediately before tools/call, one database-global transition keyed by the authorization and provider-effect digests uses authoritative database time to grant at most one owner across hosts; a consumed ambiguous attempt becomes UNKNOWN_DO_NOT_ACT instead of granting a retry. When the exact provider result and close receipt are already durable, a later process carries no MCP credential, completes only the matching global row with the persisted owner, then performs the bound terminal audit and provider-free finalization; it cannot consume or dispatch. The path has no recency fallback, rejects redirects, caps streamed responses at 256 KiB, decodes UTF-8 fatally, requires exact-one cardinality, and releases context only after both audits. Recovery evidence requires an immutable primary-ledger P-256 publisher commitment, a dedicated exact-source resolver principal, a separate exact-audit/trust resolver principal, and a separately supplied matching signing key. Both primary runner credentials must return SQLSTATE `42501` for six rollback-bounded, privilege-pure trust-root write probes and all 19 managed base-table read probes before any resolver, signing, publication, bootstrap, or MCP action. The source resolver binds the winning authority receipt to the outbox across request, proposal, logical-action, authorization, run, incident, resource, fence, effect, and payload identities; the administrator URL and synthetic signer are forbidden from both evidence runners. | Disposable-cluster validation against the exact CockroachDB Cloud version, final grants/options/membership/default-privilege inventory, MCP audit, trust-root timing, administrator and key-custody review, credential isolation, and private human review |
| Repository and evidence | Exact dependency lock, one GitHub-hosted-only normalizer invoked by exactly two protected pre-verification workflow contexts—CI before dependency installation and the manual read-only OIDC preflight before token exchange—that requires exact workflow/job/event/ref/source identity plus effective-UID/effective-GID ownership and removes only the byte-exact inert Git 2.54 `config.worktree` residue after an immediate second full checkout/config/ownership decision and before every strict caller, standalone closure-complete exact-commit build, verified local Git-bundle import, exact-Git builder and bundle inputs, bundle notices, a v7 parent-reverified 20-output privacy inventory, eight content-addressed live-drill bundles, a tracked fail-closed replacement for the optional `pg-native` peer, one pinned official Node target, a root-owned immutable-stage contract, full-history provenance, numeric Lambda targets, one exact-principal bounded evidence collector, a dedicated one-target alternate role, distinct configuration-bound Ed25519 receipt keys, fail-closed signed pre/post deployment attestation, bounded privacy scan, claim ledger, and proof manifest | Credentialed revision-fenced provider pre/post and alternate-principal receipts, key-custody review, final official-main rerun, plus private review of repository, receipts, screenshots, video, URLs, and submission fields |
