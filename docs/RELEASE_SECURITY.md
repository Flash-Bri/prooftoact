...
| CockroachDB memory and recovery | Before mutation, one read-only SERIALIZABLE v26.2 transaction binds descriptor-backed object and schema/global default privileges in the current dedicated database while recording cache-versioned principal, membership, and default-role introspection. Matching post-convergence and database-set barriers reject drift; the release boundary still requires no concurrent administrator changes. Provider dispatch v2 uses separate claim, begin, redeem, finalize, and resolve-only roles, URLs, clients, and `SECURITY DEFINER` routines with exact `session_user` guards; the migration drops the legacy polymorphic v1 transition, and resolve returns no nonce, execution capability, or completion capability. All contenders start without the MCP bearer. A claim-only broker may retry only classified pre-effect claim failures, commits `GRANTED -> EXECUTING`, and publishes the bound local execution grant only afterward. The worker receives no raw provider credential or provider client; after a create-only execution-attempt fence it invokes a nonce-bound Unix socket. Only the isolated socket-activated provider-operation broker receives the MCP bearer through systemd credentials, commits `EXECUTING -> CREDENTIAL_REDEEMED`, performs one fixed provider sequence, durably stores the exact transcript, and uses its separately derived completion capability to complete or mark unknown. Begin ambiguity and every post-redemption ambiguity fail closed without provider replay. The reconciler has only the resolve routine and cannot claim, begin, redeem, finalize, read base tables, or obtain a capability. Managed MCP remains bound to one fixed query and exact tenant, run, incident, evidence, resource, operation, request, outcome, and successor identities; it rejects redirects, caps streamed responses at 256 KiB, decodes UTF-8 fatally, requires exact-one cardinality, and releases context only after both audits. Recovery evidence retains the immutable publisher commitment, exact-source resolver, separate audit/trust resolver, and independently supplied signing key. Both recovery runner credentials must return SQLSTATE `42501` for six rollback-bounded trust-root write probes and all 20 managed base-table read probes before resolver, signing, publication, bootstrap, or MCP action. | Disposable-cluster validation against exact CockroachDB v26.2, real two-host contention and surfaced `40001`/acknowledgment ambiguity, final grants/options/membership/default-privilege inventory, systemd credential delivery, MCP audit, key-custody review, and private human review |
| Repository and evidence | Exact dependency lock; distinct no-secrets exact PR-head and synthetic-merge CI lanes; the separate protected read-only OIDC preflight; a GitHub-hosted checkout normalizer that binds each allowed workflow/job/event/ref/source identity plus effective UID/GID and removes only byte-exact inert Git 2.54 residue before strict verification; standalone closure-complete exact-commit builds; verified local Git-bundle import; exact-Git builder inputs and notices; a v9 parent-reverified 21-output inventory with nine content-addressed live-drill bundles and one pinned official Node target; twelve runtime files; a reviewed atomic root-stage installer; an independently implemented non-root verifier; eight exact systemd units; full-history provenance; numeric Lambda targets; bounded evidence roles; configuration-bound receipt keys; fail-closed deployment attestations; bounded privacy scan; claim ledger; and proof manifest | Hosted exact-head, synthetic-merge, and final official-main receipts; Linux root-stage receipt and independent verifier output; credentialed revision-fenced provider receipts; key-custody review; plus private review of repository, receipts, screenshots, video, URLs, and submission fields |
