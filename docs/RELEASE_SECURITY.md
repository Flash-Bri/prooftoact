...
| CockroachDB memory and recovery | Provider dispatch uses distinct claim/begin, redeem/finalize, activation, terminalization, resolve-only, and provider-exchange OS/database boundaries. The persistent operation broker is provider-key-isolated. After redemption it requests a fresh database activation; only the fixed one-shot exchange UID receives the MCP bearer, and only after its READY message matches the broker-acknowledged receipt does it receive `PROCEED_ONCE`. It rechecks freshness after PROCEED, creates a consumption fence, performs the fixed provider sequence, durably records the result, and then returns it. Separate database-time terminalization converges expired or ambiguous state without provider replay. Separately, the sanitized Final23 record binds a fresh-zero local QEMU-TCG x86-64 CockroachDB CCL v26.2.0 primary-capability gate: typed-definer access, direct-DML/cross-role denials, simple recovery resolution, DVI proposal-identity negatives, one protected-effect insert, and sequential exact replay passed. | Provider-hosted/external CockroachDB validation; final-head 50-session and two-host contention; held-expiry, simultaneous four-key uniqueness, and surfaced acknowledgment-ambiguity drills; integrated DVI plan/exclusions; final grants/options/membership/default-privilege inventory; complete application systemd/PID1 and root-stage credential/isolation checks; MCP audit; key-custody review; and private human review |
| Repository and evidence | Exact dependency lock; distinct no-secrets exact PR-head and synthetic-merge CI lanes; standalone closure-complete exact-commit builds; one pinned official Node target; fifteen runtime files; a reviewed atomic root-stage installer; an independently implemented non-root verifier; fourteen exact systemd units; full-history provenance; bounded evidence roles; fail-closed deployment attestations; bounded privacy scan; claim ledger; and proof manifest | Hosted exact-head, synthetic-merge, final official-main, and Linux x64 PID1 receipts; credentialed revision-fenced provider receipts; key-custody review; plus private review of repository, receipts, screenshots, video, URLs, and submission fields |
