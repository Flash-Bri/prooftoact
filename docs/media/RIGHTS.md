...
This is the controlling rights ledger for ProofToAct's website, repository,
...
The repository's MIT license applies to ProofToAct-owned source. It does not
...
| Status | Meaning |
| --- | --- |
| `CLEARED_CURRENT` | The exact hashed ProofToAct-owned source is cleared for its listed current uses. |
...
| --- | --- | --- | --- | --- | --- |
| `C03` | UI layout, palette, and component styling in `web/styles.css` — `c33554b39142780f662d9ec5fa8b8c8fd5832eb9c0e4ec5c305ee3a53fd86e9e` | ProofToAct project; newly authored in the clean-room repository beginning at commit `e198f41` | Repository MIT license | Public source repository, current local proof, signed-out AWS proof candidate, and captures of that exact proof; no attribution required. No remote image, font, or stylesheet is imported; the exact `C08 / V09` SVG is the only browser image asset. | Nunan, 2026-07-31 — `CLEARED_CURRENT` |
| `C04` | Plain-text `ProofToAct` name, UI copy, and synthetic Highwater Drill labels in `web/index.html` and `web/app.js`; file hashes `cd64c1c28d8ba0df44d49827102d91335c653056c6dbb2b8585d10130c389b21` and `825b5466035432c30598aa4b43c4a9e428d4ec88df8599cfacc25ee208a02a10` | ProofToAct project; replacement hackathon name approved by Brian on 2026-08-03 under the clean-room boundary documented in `CLEAN_ROOM.md` and the migration record in `docs/RENAME_MIGRATION.md` | Repository MIT license for source and copy only; no legal or trademark clearance claimed | Current public source and local proof copy only. A pre-existing adjacent `TideProof` repository and the bounded ProofToAct screening are recorded in `docs/PRIOR_ART.md`. A close-to-release name recheck remains required. No third-party artwork or wordmark is implied. | Nunan, 2026-08-03 — `CLEARED_CURRENT` |
| `C05` | CSS platform stacks: `ui-sans-serif`, `system-ui`, `-apple-system`, `BlinkMacSystemFont`, `"Segoe UI"`, `sans-serif`, `ui-monospace`, `SFMono-Regular`, `Menlo`, `Monaco`, `Consolas`, `monospace`; architecture SVG platform stacks also name `Helvetica` and `Arial` | Platform vendors; ProofToAct distributes no font binary, webfont, font subset, or font service request. The CSS `font` shorthands resolve only to `inherit` or the recorded monospace families. | Viewer or capture-platform software license; no separate ProofToAct font grant claimed | Runtime rendering only. A final screenshot or video receipt must name its capture platform; no font file may be bundled without a separate row and license. | Nunan, 2026-07-31 — `PLATFORM_ONLY` |
| `C06` | Plain-text references to CockroachDB, CockroachDB Cloud, Distributed Vector Indexing, Managed MCP, AWS, Amazon Bedrock, Lambda, API Gateway, KMS, IAM, GitHub, and Devpost | Marks and names belong to their respective owners; ProofToAct uses them only to identify the contest, verified integrations, or source host | Factual nominative text use; no logo, brand artwork, sponsorship, certification, or ownership claimed | Plain text in repository, local and signed-out proof UI, factual submission copy, and evidence only. Preserve official casing, pair every claim with its evidence boundary, and recheck final contest and brand rules. | Nunan, 2026-07-30 — `TEXT_ONLY_REVIEWED` |
| `C07` | Plain-text `A TrustAgentic.ai project` footer in `web/index.html` — containing-file hash `cd64c1c28d8ba0df44d49827102d91335c653056c6dbb2b8585d10130c389b21` | Relationship wording approved by Brian and recorded in `docs/VISUAL_RELEASE_SYSTEM.md`; no TrustAgentic asset is embedded or derived | Text-only relationship attribution; no trademark asset license or MIT relicensing claimed | Public source repository plus local and signed-out AWS proof surfaces. Exact text only, no hyperlink, logo, redraw, recolor, halo, chrome, sticker, or implied Northstar release. | Nunan, 2026-08-03 — `TEXT_ONLY_REVIEWED` |
| `C08 / V09` | `docs/media/architecture.svg` — `0a35e196d896e932bbadf2afbf5e915c21073c423d9d8ac8a23023ad0887c871`; `docs/media/architecture.png` — `6228172f7a5a462940a05543ee455e0de21aa53c805e9466076b9e33fed1f168` | ProofToAct project; Nunan-created standalone derivative of the clean-room `C02` trust-boundary diagram. The accessible name was amended for the approved rename on 2026-08-03; exact lineage is in `evidence/architecture-asset-rename-2026-08-03.md` | Repository MIT license | Public repository, README, website, demo video, social/press packet, and hackathon submission; no attribution required. SVG contains accessible title/description and no external asset, script, link, or embedded font. PNG is the exact `2200 × 720` RGB export rendered with macOS `/usr/bin/sips` `sips-316`; unchanged rendered pixels intentionally preserve the prior PNG digest. | Nunan, 2026-08-03 — `CLEARED_CURRENT` |
...
| --- | --- | --- | --- |
| `V01` | ProofToAct wordmark master and PNG export | `SPEC_ONLY` | `BLOCKED` until the ProofToAct-owned design, exact hashes, and lettering review have rows here. |
| `V02` | ProofToAct mark and required icon exports | `SPEC_ONLY`; deliberately omitted from the current release surfaces | `BLOCKED`; no favicon, app icon, or social avatar is published, and any future mark must pass a new exact-hash rights review rather than derive from the retired `C01` glyph. |
| `V03` | Official TrustAgentic wordmark; current custodian candidate hash `1d1aec3649f161d7e706c941baa763f73ff480b7ef09072a290373ebf2544b44` | `RIGHTS_PENDING`; not stored in ProofToAct | `BLOCKED` until an approved source is used byte-for-byte and a written grant covers every intended channel while excluding the mark from MIT relicensing. |
| `V04` | ProofToAct-owned geometric proof-field vector master | `SPEC_ONLY`; no source created | `BLOCKED` until the clean-room source, accessibility review, exact hash, claim review, and public MIT licensing are recorded. Ocean, tide, wave, shoreline, and beach imagery from the former working-name direction is retired. |
| `V05` | Deterministic responsive exports of the `V04` proof-field master | `SPEC_ONLY` | `BLOCKED` until `V04` is cleared and every derivative has a source-to-output hash chain. |
...
| `T01` | TrustAgentic.ai production homepage link | Text-only ProofToAct attribution is reviewed separately as `C07`; final destination and release state are not yet green | The production homepage link is `BLOCKED` until ProofToAct's public destination, repository, claims, rights, and release receipts are all verified. No TrustAgentic logo is authorized under this row. |
