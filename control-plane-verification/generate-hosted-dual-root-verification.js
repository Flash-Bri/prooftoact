...
import { generateHostedDualRootVerification } from
...
function parse(arguments_) {
  ...
  for (let index = 0; index < arguments_.length; index += 2) {
    ...
    if (!allowed.has(name) || values.has(name) ||
      ...
      throw new Error("HOSTED_DUAL_ROOT_ARGUMENT_REJECTED");
    }
...
async function main() {
  ...
  const manifest = await generateHostedDualRootVerification({
...
main().catch((cause) => {
  ...
  const code = /^HOSTED_DUAL_ROOT_[A-Z0-9_]{1,120}$/u.test(message)
    ? message : "HOSTED_DUAL_ROOT_UNKNOWN_HOLD";
