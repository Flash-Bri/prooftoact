...
export function renderBundledThirdPartyNotices({
  ...
  const lines = [
    ...
    "packages whose source is present in the six Gate Two Lambda bundles, the",
    "private recovery-query Lambda bundle, the evidence-provider runtime, and the",
    "integrated-live runtime bundles. It is embedded byte-for-byte in every release ZIP",
    "so each independently distributed artifact carries the complete reviewed",
    "package union and license text set.",
