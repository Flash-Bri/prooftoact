...
const EXACT_PREFLIGHT_RUNTIME_FAILURES = Object.freeze(
  Array.from({ length: EXACT_PREFLIGHT_RUNTIME_CALL_COUNT }, (_, index) =>
    Object.freeze({
...
const EXACT_PREFLIGHT_RUNTIME_BUDGET_FAILURES = Object.freeze([
  ...
  Object.freeze({ stage: "VALIDATE_BUDGET_METRICS_BASIS", exitCode: 94 }),
  Object.freeze({
    stage: "VALIDATE_BUDGET_COST_TYPES_BASIS",
    exitCode: 95
  }),
  Object.freeze({ stage: "VALIDATE_BUDGET_PERIOD_START", exitCode: 96 }),
  Object.freeze({ stage: "VALIDATE_BUDGET_PERIOD_END", exitCode: 97 }),
  Object.freeze({ stage: "VALIDATE_BUDGET_PERIOD_ORDER", exitCode: 98 }),
  Object.freeze({
  ...
  Object.freeze({ stage: "VALIDATE_BUDGET_LIMIT_UNIT", exitCode: 102 }),
  Object.freeze({
  ...
  Object.freeze({ stage: "VALIDATE_BUDGET_LIMIT_FIXED", exitCode: 105 }),
  Object.freeze({
...
const EXACT_PREFLIGHT_RUNTIME_COST_FAILURES = Object.freeze([
...
const EXPECTED_UNDERLYING_PREFLIGHT_RUNNER_SHA256 =
  "93409cfd6f553da0e6fbeceab4e7f29a7b273ed82a591692dce79e32b2724336";
const EXPECTED_UNDERLYING_PREFLIGHT_VALIDATOR_SHA256 =
  "533ca59499be1ec7d13cbfcb601e2740e0ddcb86f65a9b70d46b965e4ff08695";
...
const EXPECTED_READ_ONLY_FAILURE_STAGE_SEQUENCE_SHA256 =
  "688beddf922f0a6dd701378d68657f7b976c8d9f53307093be642493d204fb67";
const EXPECTED_READ_ONLY_FAILURE_STAGE_REFERENCE_COUNT = 204;
...
const EXPECTED_READ_ONLY_POST_DIAGNOSTIC_EXECUTION_SUFFIX_SHA256 =
  "0c83a707c34b39799dffca63748a30f722f0501df87b9c698b7f0d27cd0363ed";
...
const EXPECTED_READ_ONLY_PRE_DIAGNOSTIC_PREFIX_SHA256 =
  "160c0a00d4429b6172c99b03604ba53bbf87c51d76a97d6c188ab56b3032d372";
...
const EXPECTED_READ_ONLY_PREFLIGHT_EXIT_STAGE_MAP = Object.freeze(
  [
    ...
    ...EXACT_PREFLIGHT_RUNTIME_COST_FAILURES.slice(0, 2),
    ...
    ...EXACT_PREFLIGHT_RUNTIME_BUDGET_FAILURES,
    ...EXACT_PREFLIGHT_RUNTIME_COST_FAILURES.slice(2),
...
const EXPECTED_READ_ONLY_GPG_REFERENCE_SHA256 =
  "9bbefc04e494b4a66022be3aa0830ee1f6a60a6c6686cf970fcc34ac52126bb0";
...
function readOnlySensitiveCleanupFunction(source) {
  const block = source.match(
    /^cleanup_sensitive_files\(\) \{\n[\s\S]*?^\}$/mu
  );
  assert(block, "OIDC_READ_ONLY_RUNNER_SENSITIVE_CLEANUP_FUNCTION");
  return block[0];
}

function readOnlyPostDiagnosticExecutionSuffix(source) {
  const boundary = `${EXPECTED_READ_ONLY_DIAGNOSTIC_BLOCK}\n`;
  const boundaryIndex = source.indexOf(boundary);
  assert(
    boundaryIndex >= 0,
    "OIDC_READ_ONLY_RUNNER_POST_DIAGNOSTIC_EXECUTION_SUFFIX"
  );
  return source.slice(boundaryIndex + boundary.length);
}
...
export function validateReadOnlyRunner(source) {
  ...
  assert(
    (source.match(/\bfail_closed\b/gu) ?? []).length ===
      EXPECTED_READ_ONLY_GENERIC_FAILURE_REFERENCE_COUNT,
    "OIDC_READ_ONLY_RUNNER_GENERIC_FAILURE_REFERENCES"
  );
  assert(
    sha256(
      Buffer.from(readOnlySensitiveCleanupFunction(source), "utf8")
    ) === EXPECTED_READ_ONLY_SENSITIVE_CLEANUP_FUNCTION_SHA256,
  ...
  assert(
    sha256(
      Buffer.from(readOnlyPostDiagnosticExecutionSuffix(source), "utf8")
    ) === EXPECTED_READ_ONLY_POST_DIAGNOSTIC_EXECUTION_SUFFIX_SHA256,
    "OIDC_READ_ONLY_RUNNER_POST_DIAGNOSTIC_EXECUTION_SUFFIX"
  );
...
export function validateUnderlyingPreflight(
  ...
  assert(
    sameJson(runtimeFailures, [
      ...
      ...EXACT_PREFLIGHT_RUNTIME_CONTROL_FAILURES,
      ...EXACT_PREFLIGHT_RUNTIME_BUDGET_FAILURES,
      ...EXACT_PREFLIGHT_RUNTIME_COST_FAILURES
    ]),
  ...
  assert(
    !runnerSource.includes("String(error?.message") &&
      !runnerSource.includes("process.stderr.write(result.stderr") &&
      !runnerSource.includes("process.stdout.write(result.stderr"),
  ...
  assertMarkers(
    ...
    [
      ...
      "AWS_GATE2_PREFLIGHT_COST_FAILURE_STATE = new WeakMap()",
      "AWS_GATE2_PREFLIGHT_DIAGNOSTIC_CONTEXT_STATE = new WeakMap()",
      "createAwsGate2PreflightDiagnosticContext()",
      ...
      "costDecimalMicros(",
      "awsCostExplorerPeriodWithValidation(",
  ...
  assert(
    !validatorSource.includes(
      "export class AwsGate2PreflightBudgetFailure"
    ) &&
      !validatorSource.includes(
        "export function createAwsGate2PreflightBudgetFailure"
      ) &&
      !validatorSource.includes(
        "class AwsGate2PreflightBudgetFailure"
      ) &&
      !validatorSource.includes(
        "error instanceof AwsGate2PreflightBudgetFailure"
      ) &&
      !runnerSource.includes(
        "error instanceof AwsGate2PreflightBudgetFailure"
      ),
    "OIDC_UNDERLYING_PREFLIGHT_BUDGET_PROVENANCE"
  );
  assert(
    !validatorSource.includes(
      "export class AwsGate2PreflightCostFailure"
    ) &&
      !validatorSource.includes(
        "export function createAwsGate2PreflightCostFailure"
      ) &&
      !validatorSource.includes(
        "class AwsGate2PreflightCostFailure"
      ) &&
      !validatorSource.includes(
        "error instanceof AwsGate2PreflightCostFailure"
      ) &&
      !runnerSource.includes(
        "error instanceof AwsGate2PreflightCostFailure"
      ),
    "OIDC_UNDERLYING_PREFLIGHT_COST_PROVENANCE"
  );
  assert(
...
export const __test = Object.freeze({
  ...
  EXPECTED_READ_ONLY_PRE_DIAGNOSTIC_PREFIX_SHA256,
  EXPECTED_READ_ONLY_WORKFLOW_RUN_COMMANDS,
  EXPECTED_UNDERLYING_PREFLIGHT_RUNNER_SHA256,
  EXPECTED_ROLE_STATEMENT_SIDS,
  RECEIPT_SCHEMA,
  RECEIPT_STATUS
});
