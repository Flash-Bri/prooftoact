...
const COMMAND_SCHEMA = "prooftoact.fresh-primary-provider-command.v2";
const CONSUMPTION_SCHEMA =
  "prooftoact.fresh-primary-provider-consumption.v1";
const INTENT_SCHEMA = "prooftoact.fresh-primary-provider-intent.v2";
...
function validateIntent(value, command, consumption, authentication) {
  ...
  requireCondition(exactKeys(value, [
    ...
    value.event === "BEFORE_DATABASE_PROVIDER_DISPATCH" &&
    value.commandSha256 === command.commandSha256 &&
    value.operationId === command.operationId &&
    value.globalKeySha256 === command.globalKeySha256 &&
    value.namespaceArn === command.controllerTableArn &&
...
function validateAuthentication(value, command, observedNow) {
  ...
  requireCondition(exactKeys(value, [
    ...
    exactKeys(value.secretReadbacks, ["admin", "cloudApi", "credential"]) &&
    ...
    validateSecretReadback(value.secretReadbacks.credential,
      ...
      command.credentialBundleRawSha256), code);
  return value;
}
...
export async function runFreshPrimaryProviderController({
  ...
  const authenticationReadback = await provider.authenticate(command);
  ...
  requireCondition(occupied === null,
    "FRESH_PRIMARY_PROVIDER_OPERATION_OCCUPIED");
  ...
  const recordTransition = async (phase, payload) => {
    requireCondition(
      phase === EXPECTED_BOOTSTRAP_PHASES[sequence] &&
        plainObject(payload) &&
        payload.mutationDispatched === (sequence >= 4),
  ...
  try {
    ...
    requireCondition(
      sequence === EXPECTED_BOOTSTRAP_PHASES.length &&
        plainObject(receipt) &&
        receipt.schemaVersion ===
          "prooftoact.fresh-primary-bootstrap-receipt.v2" &&
        receipt.status === "PASS" &&
      ...
      "FRESH_PRIMARY_PROVIDER_DISPATCH_RECEIPT_REJECTED");
  } catch (cause) {
  ...
  return Object.freeze({
    schemaVersion: "prooftoact.fresh-primary-provider-controller-receipt.v2",
    status: "PASS",
    commandSha256: command.commandSha256,
    globalKeySha256: command.globalKeySha256,
    operationId: command.operationId,
    ...
    claimBoundary:
      "This receipt proves that the exact source/tree, separate Cockroach Cloud and SQL cluster identities, immutable secret-version digests, approval digest, and credential-seal digest passed through one durable provider-global intent/transition/terminal controller. It does not by itself prove the secret readbacks, Cloud API inventory, SQL bootstrap, admin deletion, root NOLOGIN posture, AWS deployment, DVI, Managed MCP, public availability, or an integrated live drill; those facts must be present in and independently verified from the bound bootstrap and provider receipts."
  });
}
