...
test("occupied global operation rejects before dispatch", async () => {
  const command = buildFreshPrimaryProviderCommand(commandInput());
  const provider = fakeProvider(command, { occupied: { status: "CONSUMED" } });
  ...
  await assert.rejects(
    ...
    /FRESH_PRIMARY_PROVIDER_OPERATION_OCCUPIED/u
  );
...
test("provider or immutable secret readback drift rejects before global state", async () => {
  ...
  assert.deepEqual(provider.events.map(([event]) => event), ["authenticate"]);
});
...
test("PASS without the exact ten-phase bootstrap chain rejects", async () => {
  const command = buildFreshPrimaryProviderCommand(commandInput());
  const provider = fakeProvider(command);
  await assert.rejects(
    runFreshPrimaryProviderController({
...
test("pre-mutation failure terminalizes without authorizing a retry", async () => {
  ...
  await assert.rejects(
    runFreshPrimaryProviderController({
      ...
      provider,
      async dispatch({ recordTransition }) {
        await recordTransition("PREFLIGHT_STARTED", {
