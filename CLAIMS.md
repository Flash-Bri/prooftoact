...
| A bounded AWS proposal path can be deployed without giving the model authority. | Local Gate Two candidate verified; live AWS proof pending | `docs/AWS_GATE2.md` and future AWS evidence receipt | Local tests validate one exact Nova request schema, proposal-only output, IAM-authenticated boundary shape, exact artifact hashes, signer-envelope bindings, local P-256 verification, and fail-closed adversarial cases. Live Bedrock, KMS, IAM, API Gateway, and CloudFormation behavior remain unverified until deployed evidence exists. |
